...
mod tests {
    ...
    async fn test_scoring_overflow_protection() {
        ...
        // Score should be clamped to valid range [0, 50]
        ...
        // The current implementation doesn't clamp, so score will be 100.0
        // This test documents the overflow behavior (not clamped to 50.0)
        assert!(
    ...
    async fn test_scoring_extreme_values() {
        ...
        oracle.set_reputation(peer1, 0).await;
        let score = compute_app_specific_score(&peer1, &oracle).await;
        assert!(
            (score - 0.0).abs() < 0.01,
            "Zero reputation should give 0.0 score"
        );

        // Test maximum valid reputation (1000)
        oracle.set_reputation(peer2, 1000).await;
        let score = compute_app_specific_score(&peer2, &oracle).await;
        assert!(
            (score - 50.0).abs() < 0.01,
            "Max reputation (1000) should give 50.0 score"
        );
    }
}
