...
pub mod pallet {
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn on_finalize(block: BlockNumberFor<T>) {
            ...
            if !events.is_empty() {
                ...
                Self::deposit_event(Event::MerkleRootPublished {
                    block,
                    root,
                    event_count,
                });
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn update_retention(
            ...
            Self::deposit_event(Event::RetentionPeriodUpdated {
                old_period,
                new_period,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn compute_merkle_root(
            ...
            let leaves: Vec<T::Hash> = events.iter().map(|e| T::Hashing::hash_of(e)).collect();
        ...
        fn prune_old_events(before_block: BlockNumberFor<T>) {
            ...
            if total_pruned > 0 {
                Self::deposit_event(Event::EventsPruned {
                    before_block,
                    count: total_pruned,
                });
            }
