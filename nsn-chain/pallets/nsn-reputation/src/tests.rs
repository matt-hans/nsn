...
fn test_merkle_root_publication() {
    new_test_ext().execute_with(|| {
        ...
        let events = events();
        let merkle_event = events.iter().find(|e| {
            matches!(
                e,
                RuntimeEvent::NsnReputation(crate::Event::MerkleRootPublished { .. })
            )
        });
...
fn test_checkpoint_creation() {
    new_test_ext().execute_with(|| {
        ...
        let events = events();
        let checkpoint_event = events.iter().find(|e| {
            matches!(
                e,
                RuntimeEvent::NsnReputation(crate::Event::CheckpointCreated { .. })
            )
        });
...
fn test_event_pruning_beyond_retention() {
    new_test_ext().execute_with(|| {
        ...
        let events = events();
        let prune_event = events.iter().find(|e| {
            matches!(
                e,
                RuntimeEvent::NsnReputation(crate::Event::EventsPruned { .. })
            )
        });
...
fn test_governance_adjusts_retention_period() {
    new_test_ext().execute_with(|| {
        ...
        let events = events();
        let update_event = events.iter().find(|e| {
            matches!(
                e,
                RuntimeEvent::NsnReputation(crate::Event::RetentionPeriodUpdated { .. })
            )
        });
...
fn test_checkpoint_truncation_warning() {
    new_test_ext().execute_with(|| {
        ...
        let events = events();
        let truncated_event = events.iter().find(|e| {
            matches!(
                e,
                RuntimeEvent::NsnReputation(crate::Event::CheckpointTruncated { .. })
            )
        });
