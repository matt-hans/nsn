...
fn test_director_work_recording() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Treasury::record_director_work(
            RuntimeOrigin::root(),
            ALICE,
            5
        ));
...
fn test_validator_work_recording() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Treasury::record_validator_work(
            RuntimeOrigin::root(),
            BOB,
            10
        ));
...
fn test_director_rewards_proportional() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Treasury::accumulated_contributions(CHARLIE).director_slots,
            0
        );
    });
}
...
fn test_validator_rewards_proportional() {
    new_test_ext().execute_with(|| {
        ...
        // Check contributions reset
        assert_eq!(
            Treasury::accumulated_contributions(ALICE).validator_votes,
            0
        );
        assert_eq!(Treasury::accumulated_contributions(BOB).validator_votes, 0);
        assert_eq!(
            Treasury::accumulated_contributions(CHARLIE).validator_votes,
            0
        );
    });
}
...
fn test_full_distribution_cycle() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            Balances::balance(&ALICE) > initial_alice,
            "Alice should receive director rewards"
        );
        assert!(
            Balances::balance(&BOB) > initial_bob,
            "Bob should receive director rewards"
        );
        assert!(
            Balances::balance(&CHARLIE) > initial_charlie,
            "Charlie should receive validator rewards"
        );

        // Check treasury balance increased (15% allocation)
        assert!(
            Treasury::treasury_balance() > initial_treasury,
            "Treasury should receive 15% allocation"
        );
...
fn test_overflow_protection_emission() {
    new_test_ext().execute_with(|| {
        ...
        for year in 1..=50 {
            ...
            assert!(
                emission.is_ok(),
                "Emission calculation should not overflow for year {}",
                year
            );
        }
    });
