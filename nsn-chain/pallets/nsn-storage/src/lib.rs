...
pub mod pallet {
    ...
    pub type PinningDeals<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        DealId,
        PinningDeal<T::AccountId, BalanceOf<T>, BlockNumberFor<T>, T::MaxShardsPerDeal>,
        OptionQuery,
    >;
    ...
    pub enum Event<T: Config> {
        ...
        /// Audit completed
        AuditCompleted { audit_id: AuditId, passed: bool },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_deal(
            ...
            // Generate deal ID
            let deal_id: DealId = T::Hashing::hash_of(&(&creator, current_block, &shards)).as_ref()
                [0..32]
                .try_into()
        ...
        pub fn calculate_audit_probability(
            ...
            let stake_info = pallet_nsn_stake::Stakes::<T>::get(&pinner);
                ...
                stake_info.role == NodeRole::SuperNode,
            ...
            let prob_per_hour_bounded =
                prob_per_hour.clamp(MIN_PROB_PER_HOUR_MILLIONTHS, MAX_PROB_PER_HOUR_MILLIONTHS);
        ...
        pub fn initiate_audit(
            ...
            let challenge = AuditChallenge {
                byte_offset: u32::from_le_bytes(random_bytes[0..4].try_into().unwrap_or([0u8; 4]))
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn verify_merkle_proof(
            ...
            let mut current_hash: [u8; 32] = T::Hashing::hash(&proof.leaf_data).as_ref()[0..32]
                .try_into()
                .unwrap_or([0u8; 32]);
            ...
            for sibling in proof.siblings.iter() {
                ...
                current_hash = T::Hashing::hash(&combined).as_ref()[0..32]
                    .try_into()
                    .unwrap_or([0u8; 32]);
        ...
        fn distribute_rewards(current_block: BlockNumberFor<T>) {
            ...
            for (deal_id, deal) in PinningDeals::<T>::iter().take(max_deals) {
                ...
                let total_pinners = deal.shards.len().saturating_mul(REPLICATION_FACTOR) as u64;
                ...
                let total_denominator = total_pinners.saturating_mul(duration_intervals);
