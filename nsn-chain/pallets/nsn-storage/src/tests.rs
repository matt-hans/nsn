...
fn test_merkle_roots(
    count: usize,
) -> BoundedVec<MerkleRoot, <Test as crate::Config>::MaxShardsPerDeal> {
...
fn submit_audit_proof_invalid_slashes() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            final_rep,
            (-50i64).max(0) as u64,
            "Reputation should be at -50 (clamped to 0) for failed audit"
        );
    });
}
...
fn audit_expiry_auto_slashes() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            final_rep,
            (-50i64).max(0) as u64,
            "Reputation should be at -50 (clamped to 0) for expired audit"
        );
    });
}
...
fn reward_distribution_works() {
    new_test_ext().execute_with(|| {
        ...
        let total_reward_distributed: u128 = (1..=5).map(|p| Pinning::pinner_rewards(p)).sum();
        ...
        assert!(
            total_reward_distributed < payment,
            "Should only distribute first interval"
        );
    });
}
...
fn claim_rewards_success_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Pinning::pinner_rewards(1),
            0,
            "Rewards should be cleared after claim"
        );
...
fn deal_expiry_updates_status() {
    new_test_ext().execute_with(|| {
        ...
        let deal_id = System::events().iter().find_map(|e| {
        ...
        assert_eq!(
            deal.unwrap().status,
            DealStatus::Active,
            "Deal should be Active initially"
        );
        ...
        let deal_expired_found = System::events().iter().any(|e| {
            matches!(
                e.event,
                RuntimeEvent::Pinning(crate::Event::DealExpired { .. })
            )
        });
        assert!(
            deal_expired_found,
            "DealExpired event should have been emitted"
        );
...
fn max_shards_boundary_works() {
    new_test_ext().execute_with(|| {
        ...
        let deal_created = events.iter().any(|e| {
            matches!(
                e.event,
                RuntimeEvent::Pinning(crate::Event::DealCreated {
                    shard_count: 20,
                    ..
                })
            )
...
fn too_many_shards_fails() {
    new_test_ext().execute_with(|| {
        ...
        let too_many_shards_result = BoundedVec::<
            ShardHash,
            <Test as crate::Config>::MaxShardsPerDeal,
        >::try_from(too_many_shards_vec);
        assert!(
            too_many_shards_result.is_err(),
            "Should fail to create BoundedVec with >20 shards"
        );

        // Test with 20 shards should succeed
        let max_shards_vec: Vec<ShardHash> = (0..20)
            .map(|i| {
                let mut shard = [0u8; 32];
                shard[0] = i as u8;
                shard
...
fn merkle_proof_structure_verification() {
    new_test_ext().execute_with(|| {
        ...
        // Test 2: Wrong leaf_index should fail
        {
            ...
            let (audit_id, _) = audits
                .iter()
                .find(|(_, audit)| audit.pinner == pinner)
                .unwrap();
...
fn valid_merkle_proof_passes() {
    new_test_ext().execute_with(|| {
        ...
        assert!(matches!(
            updated_audit.status,
            AuditStatus::Failed | AuditStatus::Passed
        ));
    });
}
...
fn reward_calculation_with_rounding() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            total_rewards <= payment,
            "Total rewards should not exceed payment"
        );
    });
}
