...
- [x] LivePortrait model loads with FP16 precision and TensorRT optimization at VortexPipeline initialization
- [x] VRAM usage for LivePortrait is 3.0-4.0 GB (measured via torch.cuda.memory_allocated())
- [x] animate() method accepts: source_image (512×512), driving_audio (45s @ 24kHz), expression_preset, fps=24, duration=45, output=video_buffer
- [x] Output is 1080×3×512×512 float32 tensor (T×C×H×W) in range [0,1]
- [x] Generation time is <8s P99 for 45-second video on RTX 3060
- [x] Lip-sync accuracy: audio-visual alignment within ±2 frames (83ms tolerance)
- [x] Expression presets supported: "neutral", "excited", "manic", "calm" (from Recipe schema)
- [x] Expression transitions are smooth (no abrupt changes between keyframes)
- [x] Outputs are written to pre-allocated video_buffer (no new allocations)
- [x] Model supports batch_size=1 (single video per call)
- [x] Error handling for: invalid image dimensions, audio length mismatch, CUDA OOM
- [x] Model determinism: same inputs + seed = identical output frames
...
**Code Complete**:
- [x] vortex/models/liveportrait.py implemented with load_liveportrait(), LivePortraitModel.animate()
- [x] FP16 precision config verified
- [x] TensorRT optimization loader (with native PyTorch fallback)
- [x] Expression preset interpolation
- [x] Audio-to-viseme pipeline
- [x] Integration with VortexPipeline._warp_video()
- [x] Output to pre-allocated video_buffer

**Testing**:
- [x] Unit tests pass (mocked LivePortrait)
- [x] Integration test generates 1080-frame video
- [x] VRAM profiling shows 3.0-4.0GB usage
- [x] Latency benchmark P99 <8s on RTX 3060
- [x] Lip-sync accuracy test passes (±2 frames)
- [x] Expression preset test shows visible differences
- [x] Audio-video length mismatch handled gracefully

**Documentation**:
- [x] Docstrings for load_liveportrait(), animate()
- [x] vortex/models/README.md updated with LivePortrait usage
- [x] VRAM budget documented (3.5GB target)
- [x] TensorRT setup instructions (optional but recommended)
- [x] Expression preset definitions documented

**Performance**:
- [x] P99 generation latency <8s for 45s video
- [x] VRAM usage stable at ~3.5GB across 100 generations
- [x] No memory leaks (VRAM delta <50MB after 100 calls)
- [x] Lip-sync accuracy >95% (±2 frames) on test dataset
