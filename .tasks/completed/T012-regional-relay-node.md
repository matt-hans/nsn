...
- [x] Binary compiles with `cargo build --release -p icn-relay`
- [x] Latency-based region detection: relay pings Super-Nodes, selects closest region
- [x] Kademlia DHT query retrieves shard manifest for video CID
- [x] QUIC client fetches shards from Super-Node (upstream connection)
- [x] LRU cache stores shards with 1TB capacity limit
- [x] Cache hit serves shard from local disk (<50ms latency)
- [x] Cache miss fetches from upstream Super-Node and caches result
- [x] QUIC server accepts viewer connections on port 9003
- [x] Viewer shard requests served within 100ms (cache hit) or 500ms (cache miss + upstream fetch)
- [x] Relay publishes availability to DHT (key: region, value: relay multiaddrs)
- [x] Graceful shutdown with cache flush (persists cached shards to disk)
- [x] Configuration loaded from TOML (cache size, upstream super-nodes, region override)
- [x] Prometheus metrics exposed on port 9103 (cache_hits, cache_misses, bytes_served, viewer_count)
- [x] Unit tests for cache eviction, latency measurement, shard fetching
- [x] Integration test: fetches shard from mock Super-Node, serves to mock viewer
