...
updated: 2025-12-26T05:52:30Z
completed_at: 2025-12-26T05:52:30Z
...
- [x] Binary compiles with `cargo build --release -p icn-super-node`
- [x] GossipSub subscription receives video chunks from directors (topic: `/icn/video/1.0.0`)
- [x] Reed-Solomon encoding creates 14 shards (10 data + 4 parity) from video chunk
- [x] Any 10 of 14 shards can reconstruct original video (verified with test data)
- [x] Shards persisted to filesystem with CID-based paths (`storage/<CID>/shard_<N>.bin`)
- [x] Shard manifests published to Kademlia DHT (key: video CID, value: JSON with shard locations)
- [x] QUIC server accepts connections from Regional Relays on port 9002
- [x] Shard retrieval API responds to relay requests within 100ms (local disk read)
- [x] Audit monitor detects pending audits from on-chain `PendingAudits` storage
- [x] Audit proof generation: read challenged bytes from shard, hash with nonce, submit extrinsic
- [x] Audit proof submission completes within 10-minute deadline (100 blocks)
- [x] Storage usage metrics tracked (total shards, bytes stored, disk utilization)
- [x] Prometheus metrics exposed on port 9102 (shard_count, bytes_stored, audit_success_rate)
- [x] Graceful shutdown with flush of pending writes
- [x] Configuration loaded from TOML (storage path, regions, chain endpoint, pinning deal monitoring)
- [x] Unit tests for Reed-Solomon encoding/decoding, shard persistence, audit proof generation
- [x] Integration test: receives mock video chunk, stores shards, retrieves via DHT
