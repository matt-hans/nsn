...
- [ ] **Direct Connection**: Attempts direct TCP/QUIC connection first (no overhead)
- [ ] **STUN Discovery**: Discovers external IP and port via STUN server
- [ ] **UDP Hole Punching**: Establishes P2P connection via coordinated UDP hole punching (ICE-like)
- [ ] **UPnP Port Mapping**: Attempts UPnP port mapping if STUN fails
- [ ] **Circuit Relay**: Fallback to libp2p Circuit Relay (v2) if UPnP unavailable
- [ ] **TURN Relay**: Ultimate fallback to TURN server for symmetric NATs
