...
- [ ] **Deduplication**: Removes duplicate PeerIds from multiple sources
- [ ] **Trust Ranking**: Ranks peers by trust level (hardcoded > DNS > HTTP > DHT)
- [ ] **Latency Ranking**: Within same trust level, ranks by ping latency
- [ ] **Fallback Logic**: Tries next layer if current layer fails completely
- [ ] **Metrics Exposed**: Bootstrap source distribution, signature verification failures
