...
class LivePortraitModel:
    ...
    def animate_from_motion(
        ...
        This method accepts motion sequences generated by external audio-to-motion
        models, bypassing the internal audio-to-viseme conversion. This provides
        higher quality lip-sync as the motion data includes full facial motion
        (not just mouth shapes).

        Args:
            source_image: Actor image from Flux, shape [3, 512, 512], range [0, 1]
            motion_data: Pre-computed motion dict from external source with keys:
        ...
        Example:
            >>> # Generate motion from audio using external model
            >>> motion_data = motion_model.generate_motion(audio, duration_sec=15.0)
        ...
        logger.info(
            "Animating from external motion data",
            extra={
        ...
        if is_delta_format:
            # Source-anchored motion adapter for relative deltas
            # This extracts source motion in LivePortrait's coordinate space
            # and applies the normalized deltas with appropriate scaling
            logger.info("Using source-anchored motion adapter for relative deltas")
    ...
    def _build_template_from_motion(
        ...
        """Build LivePortrait driving template from external motion data.

        Args:
            motion_data: External motion source output
    ...
    def _extract_source_motion(
        ...
        enabling proper delta application from external motion sources.
    ...
    def _apply_motion_deltas(
        ...
        """Apply motion deltas to source motion in LivePortrait space.

        This is the core of the source-anchored motion adapter. External motion
        sources output normalized deltas from their frame 0, and we apply these
        deltas to the source motion extracted from the actor image in LivePortrait's
        coordinate space.

        Args:
            source_motion: Motion extracted from source image (LP coordinate space)
            motion_deltas: Relative deltas from external source (normalized, from frame 0)
